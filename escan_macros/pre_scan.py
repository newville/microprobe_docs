...
def pre_scan_command(row=1, *args, **kws):
    ...
    if shutter_status != (1, 1):
        print("#pre_scan: waiting (up to 8 hours) for shutters to open")
        tx = time()
        while shutter_status != (1, 1) and (time()-tx < 8*3600.0):
            if check_scan_abort():
                break
            sleep(1.0)
            if int(time()) % 300 < 10:
    ...
    now = time()
    _scandb.set_info('prescan_lasttime', int(now))
    print(f'#pre_scan done ({now-t0:.1f} seconds)')
    return None
